...
export class Workspace {
  ...
  addTab(root, title, activate = true) {
    ...
    const tab = { id, title: title || `untitled ${this.tabSeq}`, root: root || this._make('editor'), focusedLeafId: null };
  ...
  closeTab(id) {
    ...
    if (!this.tabs.length) { this.addTab(null, 'untitled'); return; }
  ...
  renderTabs() {
    ...
    add.addEventListener('click', () => this.addTab(null, `untitled ${this.tabSeq + 1}`));
